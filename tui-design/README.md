...
- 標籤上某個固定位置被「這是哪一類 surface」訂走、不能同時拿來當純裝飾
  — 一旦兼職、那個位置就失去信號強度
...
app 用什麼符號當視覺語彙、要不要依賴 icon font、選哪個 glyph 子集、那些
符號在各種終端機與字體下實際畫幾格 —— 全部由各 app 自行決定。

**為什麼劃出去**：符號語彙的每個決定都綁在**具體環境**上 —— 哪一套 icon
font、哪些終端機、使用者裝了什麼、CJK 字型把某個 codepoint 畫成一格還是
兩格。這些條件跨 app 不一致、跨使用者也不一致；寫進通則只會產出一條某些
app 必須違反的規則（§0）。

**別的軸仍然管得到符號**（它們規範的不是「選哪個符號」）：

| 條款 | 對符號的約束 |
|---|---|
| §B 元素專職化 | 一個符號被指派語意後不能兼職 — 管的是語意分配 |
| §1.2 Width stability | 版位寬度不能跟內容綁動 — 符號寬度估錯會犯這條 |
| §4.4 Hotkey discoverability | 熱鍵要顯式標記、不能只靠符號或顏色暗示 |

各 app 的實際做法寫在它們自己的 §3。
...
使用者正在輸入文字時（form 欄位、input 框、search 輸入列），**所有會產
生字元的鍵都進入輸入框、不觸發任何動作** —— letter hotkey、`Space`、`?`
全部失效。

這不是可選的體貼、是必然：core-key 裡有兩個本身就是可列印字元（`Space`
與 `?`）。不屏蔽、使用者就打不出含空白的檔名或含問號的密碼 —— 按下空白
跳出 menu、那個字元永遠進不了輸入框。
...
| `Enter` | 送出（§A.0.K 語意不變） |
| `Tab` / 方向鍵 | 輸入面自身的欄位移動 |
| letter hotkey · `Space` · `?` | **失效**、一律當字元 |

`Esc` 與 `Enter` 不跟字元輸入競爭、而且少了它們輸入框就是出不去的陷阱
—— 「屏蔽一切」若連它們也蓋掉、user 會困在裡面。

這條是 §A.0.K「表上的鍵不能改語意」的**唯一例外**、也是 §4.2「letter
hotkey 是入口內動作的加速捷徑」在輸入態的暫停。範圍僅限文字輸入態、離
開輸入面立刻恢復。
...
1. **核心設計哲學**（§A / §B）— VTP 是目標、專職化是機制。底下所有規則
   都是這兩條的展開。
2. **七大分類**（§1~§7）— UI / UX / 互動的具體規範。其中 **§3 符號語彙
   不含規範**、是一塊明確劃給各 app 自行決定的領域（見該章）。
3. **跨類觀察**：
